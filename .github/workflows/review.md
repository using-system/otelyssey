...
5. The vendor registry, https://opentelemetry.io/ecosystem/vendors/index.md, when the relevance turns on a backend's compatibility: read it with the web-fetch tool, never from the shell, which has no network (a `curl` there is refused, not retried); when the page cannot be fetched, read the registry's source instead, the `open-telemetry/opentelemetry.io` repository under `data/registry/` (one `*.yml` per entry, `collector-exporter-<vendor>.yml` among them), through GitHub. A vendor is listed under its product's or its company's name: match either. Only when both reads leave the backend unlisted does a question to the contributor follow.
