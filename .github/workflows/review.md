---
description: Review a plugin submission whose format holds - relevance to OpenTelemetry, its category, that it is not an admitted plugin resubmitted, the conversation with the contributor, the ruling.
on:
  issues:
    types: [labeled]
    names: [format-ok]
...
2. The plugin itself at the commit `sha`, through raw.githubusercontent.com at that sha: `plugin.json`, the README, every `skills/*/SKILL.md`, `mcp.json` when it exists (the servers it declares, their `url` or `command`, are evidence of what the plugin operates: a plugin can be an MCP server and no skill), and the annex files a skill carries next to its `SKILL.md` or links from it (`references/*.md`, other Markdown or JSON in the skill's directory) when the `SKILL.md` alone does not settle the relevance. Bounds: Markdown and JSON files only, never an executable or a script; at most twenty annex files and the first two hundred lines of each; a plugin whose evidence needs more is one you ask the contributor about.
...
**Category.** One of the store's five, from the plugin's content, whatever the form says (the category the issue's form carries, when it carries one, is a suggestion you confirm or correct): `instrumentation` when the plugin makes code emit OpenTelemetry telemetry or teaches how (SDKs, auto-instrumentation, hooks that emit over OTLP); `conventions` when its subject is the semantic conventions themselves (naming, attributes, reviewing instrumentation against them); `collector` when its subject is the OpenTelemetry Collector (its configuration, pipelines, receivers, exporters, distributions); `backend` when it operates a backend that ingests OpenTelemetry telemetry (querying it, its dashboards, its alerts, its setup, its dedicated Collector exporter); `workflow` when its skills drive a process across several of the four (an observe, instrument, verify loop, a benchmark and its reading), whichever of them its text dwells on most. Otherwise a plugin that touches several takes the one its skills spend most of their text on; say which skill decided it.
...
- When the plugin is admissible and not a resubmission, comment the three rulings with their evidence, label `admissible`, and remove `under-review` when it is there. That comment's first line is exactly `` Ruling: admissible - `<name>` `<version>` at `<sha>` in `<category>` ``, with the name, the version and the full sha copied from the intake comment and the category you ruled, one of the five words above: the pipeline admits that record, in that category, and no other. It takes it from there: it opens the admission pull request and comments its link; never open one yourself.
