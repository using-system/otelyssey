...
5. The vendor registry, https://opentelemetry.io/ecosystem/vendors/index.md, when the relevance turns on a backend's compatibility: read it with the `web_fetch` tool, never from the shell, whose allowlist has no `curl` (a `curl` there is refused, and a refusal is not an absence from the registry); when, and only when, the page cannot be fetched, read its source instead, `data/ecosystem/vendors.yaml` in the `open-telemetry/opentelemetry.io` repository on its default branch, through the GitHub tools by that path, never by a code search (a search would surface any repository carrying a file of that name): one entry per vendor, its `name` and `nativeOTLP`, the same list. An entry names the product, the company, or both (`AppDynamics (Cisco)`): match the backend on either. The question to the contributor of the Relevance rule follows only when the registry, read by either route, does not list the backend.
